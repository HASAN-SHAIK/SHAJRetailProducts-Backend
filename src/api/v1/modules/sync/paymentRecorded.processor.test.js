...
describe('payment.recorded projection', () => {
  test('projects a valid payment without requiring a completed sale row', async () => {
    ...
    const result = await processPaymentRecorded(client, event);
    expect(result).toEqual({ payment_id: 'pay-1', order_id: 'ord-1', status: 'captured' });
    expect(client.query).toHaveBeenCalledTimes(1);
    expect(String(client.query.mock.calls[0][0])).toContain('INSERT INTO pos_sale_payments');
    expect(String(client.query.mock.calls[0][0])).toContain('ON CONFLICT(payment_id) DO UPDATE');
  });
