...
describe('receipt.issued projection', () => {
  test('projects an immutable receipt after its sale projection exists', async () => {
    const client = { query: jest.fn(async () => ({ rowCount: 1, rows: [] })) };

    const result = await processReceiptIssued(client, receiptEvent());
    ...
    expect(String(client.query.mock.calls[1][0])).toContain('INSERT INTO pos_sale_receipts');
    expect(client.query.mock.calls[1][1]).toContain('abc123');
  });

  test('defers receipt projection until its sale projection exists', async () => {
    const client = { query: jest.fn(async () => ({ rowCount: 0, rows: [] })) };

    await expect(processReceiptIssued(client, receiptEvent())).resolves.toEqual({
      receipt_id: 'rcp-1', order_id: 'ord-1', deferred: true, reason: 'sale_projection_missing'
    });
