...
router.post('/events', posSyncAuth, async (req, res, next) => {
  ...
  try {
    ...
    if (['INVALID_SALE_COMPLETED_PAYLOAD', 'INVALID_PAYMENT_RECORDED_PAYLOAD', 'INVALID_INVENTORY_MOVEMENT_PAYLOAD', 'INVALID_RECEIPT_ISSUED_PAYLOAD', 'INVALID_CUSTOMER_CHANGED_PAYLOAD'].includes(error.code)) {
