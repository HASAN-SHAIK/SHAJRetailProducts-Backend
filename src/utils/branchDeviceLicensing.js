...
const fetchBranchPolicy = async (tenantPool, branchId) => {
  const result = await tenantPool.query(
    `SELECT id, subscription_plan, max_devices_allowed
     FROM branches
     WHERE id = $1`,
    [branchId]
  );
...
const logDeviceEvent = async (tenantPool, payload) => {
  try {
    ...
    // Best-effort logging. Do not block requests if logging fails.
    console.error('Failed to log device event:', error.message || error);
  }
};

const ensureDeviceRegistration = async ({
  tenantPool,
  branchId,
  deviceId,
  userId,
  ...
  if (!branchId || !deviceId) {
    return { allowed: false, code: 'DEVICE_CONTEXT_MISSING' };
  }

  const branch = await fetchBranchPolicy(tenantPool, branchId);
  if (!branch) {
    return { allowed: false, code: 'BRANCH_NOT_FOUND' };
  }
  ...
  const existingRes = await tenantPool.query(
    `SELECT id, is_active
     FROM branch_devices
     WHERE branch_id = $1 AND device_id = $2`,
    [branchId, deviceId]
  );
      ...
      if (maxAllowed !== null) {
        ...
        if (countRes.rows[0].count >= maxAllowed) {
          await logDeviceEvent(tenantPool, {
            ...
            metadata: { reason: 'limit_reached', limit: maxAllowed }
          });
          return { allowed: false, code: 'DEVICE_LIMIT_REACHED', limit: maxAllowed };
        }
      }
      if (mode === 'validate') {
        return { allowed: false, code: 'DEVICE_INACTIVE' };
      }
      await tenantPool.query(
  ...
  if (mode === 'validate') {
    return { allowed: false, code: 'DEVICE_NOT_REGISTERED', limit: maxAllowed };
  }
...
const sanitizeDeviceContext = (req) => {
  ...
  const deviceInfo = {
    device_name: req.body?.device_name || req.headers['x-device-name'] || null,
    browser_info: userAgent || null,
    os_info: req.headers['x-os-info'] || null,
