...
const getBranchDevices = async (req, res) => {
  try {
    ...
    if (!branchId) {
      return jsonError(res, 400, 'VALIDATION_ERROR', 'branch_id is required');
    }
...
const deactivateBranchDevice = async (req, res) => {
  try {
    ...
    if (!branchId || !deviceId) {
      return jsonError(res, 400, 'VALIDATION_ERROR', 'branch_id and device_id are required');
    }
    ...
    const deviceRes = await req.tenantPool.query(
      `UPDATE branch_devices
       ...
       RETURNING id, device_id, is_active`,
      [deviceId, branchId]
    );
    ...
    await req.tenantPool.query(
      ...
      [
        ...
        'DEVICE_DEACTIVATED',
        JSON.stringify({ by: req.user?.user_id || req.user?.id || null })
      ]
    );
...
const updateBranchPlan = async (req, res) => {
  try {
    ...
    const maxAllowed =
      max_devices_allowed === null || max_devices_allowed === undefined
        ? null
        : Number(max_devices_allowed);

    if (!plan) {
      return jsonError(res, 400, 'VALIDATION_ERROR', 'subscription_plan is required');
    }
    ...
    await req.tenantPool.query(
      ...
      [
        ...
        null,
        'BRANCH_PLAN_UPDATED',
        JSON.stringify({ subscription_plan: plan, max_devices_allowed: maxAllowed })
      ]
    );
...
const registerDeviceOnBranch = async (req, res) => {
  try {
    ...
    if (!deviceId) {
      return jsonError(res, 400, 'VALIDATION_ERROR', 'device_id is required');
    }
    ...
    return jsonOk(res, { success: true, limit: result.limit }, 'Device registered');
  } catch (error) {
