...
describe('offline POS grant issuer', () => {
  ...
  test('signs a POS-only grant with Central private key that the public key verifies', async () => {
    const req = {
      user: {
        user_id: '44',
        tenant_id: 'tenant-1',
  ...
  test('fails closed when Central signing key is not configured', async () => {
    ...
    await issueOfflineGrant({ user: { user_id: '44', tenant_id: 'tenant-1', role: 'cashier' } }, res);
