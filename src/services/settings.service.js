...
const ensureSettingsRow = async (requestPool) => {
  const existing = await requestPool.query(
    'SELECT id, whatsapp_bill_module FROM settings ORDER BY id ASC LIMIT 1'
  );
  if (existing.rowCount > 0) return existing.rows[0];

  const inserted = await requestPool.query(
    'INSERT INTO settings (whatsapp_bill_module) VALUES (FALSE) RETURNING id, whatsapp_bill_module'
  );
