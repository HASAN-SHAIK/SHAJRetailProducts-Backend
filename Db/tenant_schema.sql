...
CREATE TABLE IF NOT EXISTS orders (
  ...
  source_completed_by_user_id TEXT,
  created_at TIMESTAMP DEFAULT (NOW() AT TIME ZONE 'UTC'),
  updated_at TIMESTAMP DEFAULT (NOW() AT TIME ZONE 'UTC'),
  is_deleted BOOLEAN DEFAULT FALSE,
...
CREATE TABLE IF NOT EXISTS order_items (
  ...
  source_discount_minor BIGINT,
  taxable_minor BIGINT,
  gst_rate_bps BIGINT CHECK (gst_rate_bps IS NULL OR (gst_rate_bps >= 0 AND gst_rate_bps <= 10000)),
  tax_minor BIGINT,
...
-- POS central sync inbox and projections.
